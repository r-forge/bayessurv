...
function(Data, ststart, stend, model="SI", niter=50000, burnin=5001, thinning=50, rptp = ststart, jumps=NULL, ini.pars=NULL, priors=NULL, lifetable = TRUE, datacheck=TRUE){
	...
	test.pj     = unlist(lapply(1:nz, function(ii) TestIniPars(Data=Data, ini.pars=thg[ii,], jumps=thj[ii,], ststart=ststart, stend=stend, model=model)[c("Pars","Jumps")]))
	idchp       = which(test.pj[rep("Pars", nz)])
	idchj       = which(test.pj[rep("Jumps", nz)])
	if(length(idchp)>0) cat("\nInitial parameters produce pdf values equal to 0 for some individuals.\nModify initial parameters and run model again.\n")
	if(length(idchj)>0) cat("\nJumps produce pdf values equal to 0 for some individuals.\nReduce jumps and run model again.\n")

	
	if(length(which(!test.pj))==length(test.pj)){
		runmcmc     = TRUE
...
#				dgibbs     = rbind(dgibbs,dg)
		...
		if(!naflag){
			...
			if(lifetable) LT = CohortLT(xq[1,bq[1,]>=Ti],ax=0.5,n=1) else LT = NULL
	...
	output = list(bd = bd,Y = Y,Z = Z, last.step=g, niter=niter, burnin = burnin, thinning = thinning, theta=thgibbs, pi = pigibbs, bis = bgibbs, xis = dgibbs-bgibbs, post=postm, thsum = thq, pisum=piq, xqsum=xq, Sxsum = Sxq, mxsum = mxq, modm=modm, idm=idm, jumps=jumps, ini.pars=ini.pars, priors=priors,  full.run=full.run, ModSel = ModSel)
	if(lifetable) output$LT = LT
	return(output)
}
